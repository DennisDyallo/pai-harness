...
import type { Sandbox, SandboxOptions, FileChange } from './types';
...
export function createSandbox(options: SandboxOptions = {}): Sandbox {
  const dir = options.paiDir ?? join(tmpdir(), `pai-harness-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    function walk(current: string) {
      for (const entry of readdirSync(current, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          try { result.set(fullPath, readFileSync(fullPath, 'utf-8')); } catch {}
        }
      }
    }
    walk(dirPath);
