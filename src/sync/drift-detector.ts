...
export function detectDrift(
  ...
  const src = sourceDir ?? join(process.env.HOME ?? '', 'Code', 'pai-private', 'config', 'hooks');
  ...
  report.hasDrift = report.modified.length > 0 || report.added.length > 0 || report.removed.length > 0;
  return report;
}
...
export function formatDriftReport(report: DriftReport): string {
  ...
  if (report.identical.length) {
    lines.push(`Identical (${report.identical.length}): ${report.identical.join(', ')}`);
  }
  if (report.modified.length) {
    lines.push(`Modified  (${report.modified.length}): ${report.modified.join(', ')}`);
  }
  if (report.added.length) {
    lines.push(`Live-only (${report.added.length}): ${report.added.join(', ')}`);
  }
  if (report.removed.length) {
    lines.push(`Source-only (${report.removed.length}): ${report.removed.join(', ')}`);
  }
