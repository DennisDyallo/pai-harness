...
export function validateHookOutput(output: unknown): ValidationError[] {
	...
	if ("decision" in obj) {
		if (
			...
			!(VALID_DECISIONS as readonly string[]).includes(obj.decision)
		) {
			errors.push({
				field: "decision",
				message: `Must be one of: ${VALID_DECISIONS.join(", ")}`,
				value: obj.decision,
			});
		}
	}
	...
	if ("hookSpecificOutput" in obj) {
		...
		if (typeof hso !== "object" || hso === null) {
			...
			if (
				!("hookEventName" in hsoObj) ||
				typeof hsoObj.hookEventName !== "string"
			) {
				errors.push({
					field: "hookSpecificOutput.hookEventName",
					message: "Required string field",
					value: hsoObj.hookEventName,
				});
			}
			if ("permissionDecision" in hsoObj) {
				if (
					typeof hsoObj.permissionDecision !== "string" ||
					!(VALID_PERMISSION_DECISIONS as readonly string[]).includes(
						hsoObj.permissionDecision,
					)
				) {
					errors.push({
						field: "hookSpecificOutput.permissionDecision",
						message: `Must be one of: ${VALID_PERMISSION_DECISIONS.join(", ")}`,
						value: hsoObj.permissionDecision,
					});
				}
			}
			if (
				"permissionDecisionReason" in hsoObj &&
				typeof hsoObj.permissionDecisionReason !== "string"
			) {
				errors.push({
					field: "hookSpecificOutput.permissionDecisionReason",
					message: "Must be a string",
					value: hsoObj.permissionDecisionReason,
				});
			}
			if (
				"updatedInput" in hsoObj &&
				(typeof hsoObj.updatedInput !== "object" ||
					hsoObj.updatedInput === null)
			) {
				errors.push({
					field: "hookSpecificOutput.updatedInput",
					message: "Must be an object",
					value: hsoObj.updatedInput,
				});
			}
			if (
				"additionalContext" in hsoObj &&
				typeof hsoObj.additionalContext !== "string"
			) {
				errors.push({
					field: "hookSpecificOutput.additionalContext",
					message: "Must be a string",
					value: hsoObj.additionalContext,
				});
			}
		}
	}
