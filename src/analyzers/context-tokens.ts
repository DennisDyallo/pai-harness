...
export function formatBudgetTable(budget: ContextBudget): string {
  ...
  for (const piece of budget.pieces.sort((a, b) => b.estimatedTokens - a.estimatedTokens)) {
    const name = piece.source.length > 34 ? piece.source.slice(0, 31) + '...' : piece.source;
    lines.push(
  ...
  lines.push(`${'Budget'.padEnd(35)} ${budget.budgetTokens.toLocaleString().padStart(8)}`);
  lines.push('');
