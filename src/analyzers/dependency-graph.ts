...
const FS_READ_PATTERN =
	/readFileSync\s*\(\s*(?:join\s*\([^)]*\)|`[^`]*`|'[^']*'|"[^"]*")/g;
const FS_WRITE_PATTERN =
	/writeFileSync\s*\(\s*(?:join\s*\([^)]*\)|`[^`]*`|'[^']*'|"[^"]*")/g;
...
export function generateGraph(hooksDir?: string): DependencyGraph {
		...
		for (const writePath of writer.writes) {
			for (const reader of nodes) {
				...
				for (const readPath of reader.reads) {
					if (
						readPath === writePath ||
						readPath.includes(writePath) ||
						writePath.includes(readPath)
					) {
						edges.push({
							from: writer.name,
							to: reader.name,
				...
				for (const checkPath of reader.checks) {
					if (
						checkPath === writePath ||
						checkPath.includes(writePath) ||
						writePath.includes(checkPath)
					) {
						edges.push({
							from: writer.name,
							to: reader.name,
	...
	const uniqueEdges = edges.filter(
		(e, i, arr) =>
			arr.findIndex(
				(x) => x.from === e.from && x.to === e.to && x.via === e.via,
			) === i,
	);
...
export function printAsciiGraph(graph: DependencyGraph): string {
	...
	lines.push(
		`${graph.nodes.length} hooks, ${graph.edges.length} dependencies\n`,
	);
...
export function checkDependencies(hooksDir?: string): DependencyCheck {
	...
	for (const node of graph.nodes) {
		for (const readPath of node.reads) {
			const hasWriter = graph.edges.some(
				(e) => e.to === node.name && e.via === readPath,
			);
