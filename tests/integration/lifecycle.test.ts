...
const HOOKS = {
  ...
  CapabilityRecommender: join(HOME, '.claude/hooks/CapabilityRecommender.hook.ts'),
  ...
  WorkCompletionLearning: join(HOME, '.claude/hooks/WorkCompletionLearning.hook.ts'),
...
describe('Full Session Lifecycle', () => {
  ...
  it('Step 2: UserPromptSubmit (CapabilityRecommender) exits 0', async () => {
    const input = makeUserPromptInput('Refactor the auth module to use OAuth2', {
  ...
  it('Step 3: PreToolUse (SecurityValidator) allows safe Bash', async () => {
    const input = makePreToolUseInput('Bash', { command: 'echo hello world' }, {
      session_id: sessionId,
    });
    const result = await runHook({
