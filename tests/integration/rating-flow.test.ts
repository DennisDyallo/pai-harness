...
const LAST_RESPONSE_HOOK = join(
	HOME,
	".claude/hooks/LastResponseCache.hook.ts",
);
...
describe("Rating Flow: LastResponseCache → RatingCapture", () => {
	...
	beforeEach(() => {
		...
		sandbox.env.PAI_INFERENCE_MOCK = join(
			sandbox.dir,
			"mock-inference-response.json",
		);
	});
		...
		for (const cat of categories) {
			...
			for (const sub of readdirSync(catDir, { withFileTypes: true })) {
				if (sub.isDirectory()) {
					...
					if (
						files.some(
							(f: string) => f.includes("LEARNING") && f.includes("low-rating"),
						)
					) {
	...
	it("should handle explicit rating without prior response cache", async () => {
		...
		const entry = JSON.parse(
			readFileSync(ratingsPath, "utf-8").trim().split("\n")[0],
		);
