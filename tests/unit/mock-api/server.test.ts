...
import { startMockServer, stopMockServer, type MockServer } from '../../../src/mock-api/server';
...
describe('mock-api server', () => {
  ...
  it('returns non-streaming message response', async () => {
    ...
    expect((body.content as Array<{ type: string; text: string }>)[0].text).toContain('mock Claude response');
  });
  ...
  it('throws on unknown scenario', () => {
    expect(() => startMockServer({ port: 9911, scenario: 'nonexistent' })).toThrow('Scenario not found');
  });
