...
switch (command) {
	...
	case "validate": {
		...
		if (subcommand === "drift") {
			...
			console.log(
				`Hooks: ${result.hookCount}, Sections: ${result.sectionsFound.join(", ")}`,
			);
			...
			console.log(
				`State files: ${valid} valid, ${invalid} invalid (${files.length} total)`,
			);
	...
	case "graph": {
		const { generateGraph, printAsciiGraph, checkDependencies } = await import(
			"../src/analyzers/dependency-graph"
		);
	...
	case "logs": {
		const { listSessions, parseSession, filterHookEntries, filterErrors } =
			await import("../src/live/log-parser");
		...
		if (sessionFlag !== -1 && args[sessionFlag + 1]) {
			...
			if (hooksOnly) {
				...
				for (const h of hooks)
					console.log(`[${h.timestamp}] ${h.hookName} (${h.event})`);
				...
				for (const e of entries)
					console.log(
						`[${e.timestamp}] ${e.type}: ${JSON.stringify(e.content).slice(0, 120)}`,
					);
				...
				for (const s of sessions.slice(0, 20)) {
					const size =
						s.sizeBytes > 1024
							...
							: `${s.sizeBytes}B`;
					console.log(
						`  ${s.id}  ${s.date.toISOString().slice(0, 19)}  ${size}`,
					);
				}
	...
	case "live": {
		...
		if (subCmd === "tail") {
			...
			if (!logPath) {
				console.error(
					"PAI_HARNESS_LOG env var not set. Set it to a file path to enable logging.",
				);
				process.exit(1);
			}
				...
				for (const line of text.split("\n")) {
					...
					try {
						...
						const code =
							entry.exitCode === 0
								? "\x1b[32m0\x1b[0m"
								: `\x1b[31m${entry.exitCode}\x1b[0m`;
						console.log(
							`\x1b[90m${entry.ts}\x1b[0m \x1b[36m${entry.hook}\x1b[0m [${entry.event}] exit=${code} ${entry.durationMs}ms`,
						);
					} catch {
	...
	case "replay": {
		...
		for (const r of results) {
			...
			console.log(
				`  ${status} ${r.hook} [${r.event}] original=${r.originalExitCode} replay=${r.replayExitCode} ${r.durationMs.toFixed(0)}ms`,
			);
			if (r.match) matched++;
		}
	...
	case "mock-api": {
		...
		if (subCmd === "start") {
			...
			console.log(
				"Mock API server runs in the foreground. Use Ctrl+C to stop it.",
			);
		} else {
			console.error(
				"Usage: pai-harness mock-api start [--port N] [--scenario name]",
			);
			process.exit(1);
		}
		break;
	...
	case "bench":
		console.log(
			`[pai-harness] '${command}' — not yet implemented. Coming in Phase 6+.`,
		);
		process.exit(0);
		break;
